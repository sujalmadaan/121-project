...
# -----------------------------
# ACTIVITY COEFFICIENTS (WITH SALINITY)
# -----------------------------
def compute_gamma(x_CO2, T, m):
    ...
    # --- Salinity correction (Setchenov) ---
    k_s = 0.12  # salting-out constant

    gamma_CO2 = gamma_CO2 * np.exp(k_s * m)

    # water activity approx = 1
    gamma_H2O = gamma_H2O
...
#     for m in salinities:
#         P, x_vals, _ = run_pressure_sweep(T, m)
#         plt.plot(P, x_vals, label=f"m = {m} mol/kg")

#     plt.xlabel("Pressure (bar)")
#     plt.ylabel("CO2 in Water (mol%)")
#     plt.title(f"Salinity Effect at T = {T}°C")
#     plt.legend()
#     plt.grid()
#     plt.show()
...
def plot_paper_style():
    ...
    for i, T in enumerate(temps):
        ...
        for m in salinities:
            ...
            # label near end of curve (like paper)
            ax.text(
                pressures[-1],
                molality_vals[-1],
                f"{m} m",
